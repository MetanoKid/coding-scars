...
excerpt: "Convert a Windows Application into a Class Library we can use from other projects"
author: Meta
category: Toolbox
tags:
  - Log
...
Now we'll have a new project with a new autogenerated _Program.cs_ file. We'll keep it, and modify its `Main` to something familiar:
...
Now, we can happily run our solution and the formatted log message will show up in the console. We're getting there!
...
Let's add a new class called `App` in our `LogWindowUI` project with this code:
...
With this, all errors will go away and we can run our console-based _hello log_. How do we connect them, now?
...
This way, we control the life cycle ourselves. Let's also add a new method to test it:
...
And it works as expected, yay! Now, the window!
...
We've defined our `LoggerUI` singleton class as the entry point to our WPF library. We could have the _Application_ as a member and tie its creation to the creation of the singleton itself. Something like this:
...
When we run this code, it's not guaranteed the _Application_ and its _Window_ are running after we call `LoggerUI.Initialize()` (which ends up calling the constructor and thus this code). If we were to start logging stuff right after calling `Initialize` it could fail because `m_application` could be uninitialized.

One way of solving this issue is to wait for the _Application_ to be ready. After all, it's part of the initialization process. We can use an `AutoResetEvent` to wait in our _main thread_ until the _Application_ and _Window_ are running. We could rewrite the constructor as such:
...
All that's left is sending messages from the host program to our WPF library!
...
And we create `MainWindow.AddLogEntry` as such:
...
Yay! It works! Looks like we did it, good job!

---

We've managed to convert our WPF project into a `Class Library` we can plug into other projects and log whatever we want from that project. We've defined a Singleton entry point for the library and created the _UI Thread_ in which the _Application_ and _Window_ live.
