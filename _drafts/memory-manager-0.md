...
I went on and created a _copy_ of the default `Allocator`: it forwarded memory requests to `new` and `delete`. Not bad, as I could create other allocators that obtained that dynamic memory from other source instead of asking for it to the OS. After that I created another `Allocator` that would get the memory from other source.
...
Let me give you a free piece of advice that I find very useful. Something I learned some time after I started learning Computer Science. Something I was told when I started programming but I didn't understand by then.
...
Before we draw a diagram with how we'll structure our system, let's stop a second and think about how we want the user to interact with it. We need to expose some functions to the user so she can do what we just defined. We're building it in C++, so we can define the following class:
...
One of the requirements we set ourselves was that the memory we manage is finite. That means the `MemoryManager`, somehow, has access to a chunk of the total available memory. Then, we slice it into chunks of different sizes based on what is requested. So, we need to keep track of all of the chunks we've sliced the memory into.
...
Knowing all this, we can finally start coding! But that will be in the next post in the series.
